...
pub fn draw_ai_sidebar(frame: &mut Frame, app: &App, area: Rect) {
    ...
    if !app.pending_ai_commands.is_empty() {
        let mut pending_rows = vec![
            Line::from(vec![
                Span::styled(" ⚠ Pending AI Actions:", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))
            ]),
        ];
        for (i, cmd) in app.pending_ai_commands.iter().enumerate() {
            let text = match cmd {
                crate::app::AgentCommand::Create { path, .. } => format!("  [{}] CREATE {}", i + 1, path),
                crate::app::AgentCommand::Append { path, .. } => format!("  [{}] APPEND {}", i + 1, path),
                crate::app::AgentCommand::Read { path } => format!("  [{}] READ {}", i + 1, path),
                crate::app::AgentCommand::Delete { path } => format!("  [{}] DELETE {}", i + 1, path),
                crate::app::AgentCommand::Rename { old, new } => format!("  [{}] RENAME {} -> {}", i + 1, old, new),
                crate::app::AgentCommand::List { path } => format!("  [{}] LIST {}", i + 1, path),
                crate::app::AgentCommand::Test { command } => format!("  [{}] TEST {}", i + 1, command),
                crate::app::AgentCommand::Commit { message } => format!("  [{}] COMMIT \"{}\"", i + 1, message),
            };
            pending_rows.push(Line::from(Span::styled(text, Style::default().fg(Color::Yellow))));
        }
        pending_rows.push(Line::from(""));
        pending_rows.push(Line::from(Span::styled(" Press Alt+Shift+Enter to Approve All ", Style::default().bg(Color::Yellow).fg(Color::Black).add_modifier(Modifier::BOLD))));
